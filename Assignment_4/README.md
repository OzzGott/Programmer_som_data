...
#### Exercise 5.7
Added `TypL` to TypedFun/TypedFun.fs to represent the List containing elements of type `typ`.
...
In ex3 (Fun1/ParseAndRunHigher.fs), the inner `let x = 77` does not affect the result. The function returned by `add 2` has already stored the value `2` for `x`, so `addTwo 5` uses `2` and not `77`. Therefore, the expression evaluates to `7 (2 + 5)`.

As for ex4, the call to `add` with argument `2` will return the function `f` with `f y = 2 + y`. Specifically it returns the closure representing:
`Closure("f", "y", x + y, [(x, 2)])`
...
#### Exercise 6.3
- added examples at the bottom of Fun1/ParseAndRunHigher.fs
